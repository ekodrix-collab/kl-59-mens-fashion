...
export default function ShopProductPage() {
  ...
  const { productsQuery, getProductBySlug } = useProducts()
  ...
  const related = allProducts
    ...
    ?.filter(p => p.product_categories?.some((pc: any) => pc.category_id === primaryCat?.id))
    ?.slice(0, 4) || []
    ...
    return (
      <main className="min-h-screen bg-black flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <span className="font-sans text-[10px] uppercase tracking-[0.3em] text-white/30">Loading Product</span>
        </div>
      </main>
    )
  }
  ...
  if (isError || !product) {
    return (
      <main className="min-h-screen bg-black flex flex-col items-center justify-center text-center px-10">
        <h1 className="font-display text-5xl text-white mb-6">Not <span className="italic">Found</span></h1>
        <p className="font-sans text-[10px] uppercase tracking-[0.2em] text-white/40 mb-10">
          This product may have been removed or the link is incorrect.
        </p>
        <Link href="/shop" className="px-10 py-4 border border-white/20 text-white font-sans text-[10px] uppercase tracking-[0.3em] hover:border-gold hover:text-gold transition-all">
          Return to Catalogue
        </Link>
      </main>
    )
